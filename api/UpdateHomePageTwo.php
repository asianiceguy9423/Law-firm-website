...
if($method === 'GET'){
  if(isset($_GET['id'])){
    ...
    //得到資料庫檔案，並將檔案交給前端
    $sql = "SELECT * FROM tbl_home_page_two WHERE id=$id";
...
if ($method ==='POST') {
   ...
   // 獲取前端数据
   $id = $_POST['id'];
   $HomeTeoTitle = $_POST['HomeTeoTitle'];
   $HomeTeoExp = $_POST['HomeTeoExp'];
   $currentHomeTwo_image = $_POST['currentHomeTwo_image'];
   $image_name = $currentHomeTwo_image;
    ...
    if (isset($_FILES['image']['name']) && $_FILES['image']['name'] !=""){
      ...
      if($currentHomeTwo_image !=""){
        $remove_path = "../src/assets/home/" . $currentHomeTwo_image;
  ...
  //連接到資料庫並更新image_name
  $sql ="UPDATE tbl_home_page_two SET
