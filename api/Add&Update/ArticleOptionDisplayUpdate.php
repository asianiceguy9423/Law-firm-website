...
try {  
    // ✅ 解析 JSON (接收純陣列)
    $ArticleOption = json_decode(file_get_contents('php://input'), true);

    // ✅ 確保 JSON 是陣列且長度為 4
    if (!$ArticleOption || !is_array($ArticleOption) || count($ArticleOption) !== 5) {
        echo json_encode(['success' => false, 'message' => 'JSON 格式錯誤']);
        exit;
    }

    // ✅ 允許的 number 值
    $allowedArticleOptionNumber = ["one", "two", "three", "four", "five"];

    // ✅ 驗證每個物件
    foreach ($ArticleOption as $index => $ArticleOptionItem) {
        if (!isset($ArticleOptionItem['name'], $ArticleOptionItem['number'], $ArticleOptionItem['img'], $ArticleOptionItem['banner'], $ArticleOptionItem['display'], $ArticleOptionItem['explain'])) {
            echo json_encode(['success' => false, 'message' => "ArticleOption配置第 {$index} 筆格式錯誤"]);
            exit;
        }        

        // ✅ 驗證 `number` 是否在允許列表內
        if (!in_array($ArticleOptionItem['number'], $allowedArticleOptionNumber, true)) {
            echo json_encode(['success' => false, 'message' => "number `{$ArticleOptionItem['number']}` 不允許"]);
            exit;
        }

        // ✅ 確保 display 為 `true` 或 `false`
        if (!is_bool($ArticleOptionItem['display'])) {
    ...
    // ✅ 確保是 JSON 字串
    $ArticleOptionJson = json_encode($ArticleOption, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
