...
try {  
    // ✅ 解析 JSON (接收純陣列)
    $StoreInfo = json_decode(file_get_contents('php://input'), true);
    ...
    // ✅ 驗證每個物件
    foreach ($StoreInfo as $index => $storeInfoItem) {
        if (!isset($storeInfoItem['name'], $storeInfoItem['title'], $storeInfoItem['detail'], $storeInfoItem['display'])) {
            echo json_encode(['success' => false, 'message' => "storeInfo配置第 {$index} 筆格式錯誤"]);
            exit;
        }        

        // ✅ 驗證 `name` 是否在允許列表內
        if (!in_array($storeInfoItem['name'], $allowedStoreInfoName, true)) {
            echo json_encode(['success' => false, 'message' => "name `{$storeInfoItem['name']}` 不允許"]);
            exit;
        }

        // ✅ 確保 display 為 `true` 或 `false`
        if (!is_bool($storeInfoItem['display'])) {
    ...
    if ($count > 0) {
        ...
        $stmt = $conn->prepare("UPDATE tbl_basic SET 
            ...
            WHERE id = ?
        ");
        $stmt->bind_param("si", $StoreInfoJson, $id);
        ...
        // **新增資料**
        $stmt = $conn->prepare("INSERT INTO tbl_basic (basic_store_Info) VALUES (?)");
        $stmt->bind_param("s", $StoreInfoJson);
        $stmt->execute();
