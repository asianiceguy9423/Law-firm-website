      ...
      <div class="ObjectBox ObjectGrid">
        <tr class="ObjectItem">
          ...
          <td>
            <input type="text" 
            :value="title"
            @input="$emit('update:title', $event.target.value)" placeholder="這邊輸入標題" 
            class="InputText">
          </td>
        </tr>
        <tr class="ObjectItem">
          ...
          <td>
            <textarea 
            :value="writings" 
            @input="$emit('update:writings', $event.target.value)" 
            placeholder="這邊輸入內文" 
        ...
        <tr class="ObjectItem">
          <td>選擇圖片</td>
          <td>
            <input class="fileInput fileInputPersonImage" 
            type="file" 
...
export default {
  ...
  computed: {
    currentImageUrl() {
      return `${this.$apiurl}/src/assets/about/${this.currentImage}`;
    }
  },
  methods:{    
    ...
    triggerFileInput(){
      this.$el.querySelector('.fileInputPersonImage').click();
    },
  }
}
</script>
