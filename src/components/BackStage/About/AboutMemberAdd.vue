  ...
  <div class="BK">
    ...
    <div class="FlexBoxBottom PaddingLeftRight">
      ...
      <div>
        ...
        <button @click="submitAboutMemberAdd" type="submit" class="ItemButton">確定新增</button>
      </div>
    </div>
...
export default {
  ...
  setup(){
    ...
    const submitAboutMemberAdd = async () => {
      ...
      try {
        const response = await axios.post(`${apiUrl}/api/Add&Update/AboutMemberAdd.php`, formData);
        ...
        eventBus.emit('sentErrorMessage', error?.message || '發生錯誤');
